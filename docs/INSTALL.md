...
The script checks every page, every static asset, every JSON endpoint, the `/api/feed` JSON service surface, the pro response headers, and that the streaming endpoint reaches the configured zone.
...
curl -sI "$YOUR_URL/api/feed" | grep -iE 'x-request-id|server-timing'
```
...
In your existing client config, **change only the `host` field** to the new Vercel URL. Everything else (UUID, `path=/api/feed`, SNI, ALPN, fingerprint) stays the same.
...
| `/api/feed` returns HTML 404 | check `ROUTE` env var (or unset it to use the default `/api/feed`) |
