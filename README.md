...
Client ─── TLS / SNI: vercel ───► │                             │
        XHTTP request to /api/feed/…  │  /api/feed/<session>/...  ───►──┼──► your Xray (TARGET_DOMAIN)
        (or any other URL)        │                             │
...
- A request is identified as proxy traffic only if its path starts
  with the configured **proxy prefix** (default `/api/feed`) **and** has a
  non-empty session segment after the prefix (e.g.
  `/api/feed/abcd1234/...`). This is exactly the shape of every real Xray
  XHTTP request — Xray always appends a session UUID to the configured
  path.
- A bare GET to `/api/feed`, `/api/feed/`, `/api`, or anything else returns
...
- A working **Xray server with XHTTP inbound** already running on a
  ...
  `https://panel.mahandevs.com:8080`.
...
| --------------- | -------- | -------------------------------- | ------------------------------------------------------------------------------------------------- |
| `TARGET_DOMAIN` | yes      | `https://panel.mahandevs.com:8080`  | Full origin URL of your backend Xray XHTTP endpoint.                                              |
| `PROXY_PATH`    | no       | `/api/feed`                          | Path prefix that triggers proxy mode. Must match the `path` field of the Xray inbound. Default `/api/feed`. |
...
```
vless://0a285ffd-f3c0-47fe-bfbd-b01711c8c5a3@react.dev:443?encryption=none&security=tls&sni=react.dev&fp=chrome&alpn=h2%2Chttp%2F1.1&insecure=0&allowInsecure=0&type=xhttp&host=your-app.vercel.app&path=%2Fapi%2Ffeed&mode=auto&extra=%7B%22xPaddingBytes%22%3A%22100-1000%22%7D#abc-relay
```

The `path=/api/feed` segment must equal `PROXY_PATH` on the relay (which
also defaults to `/api/feed`).
