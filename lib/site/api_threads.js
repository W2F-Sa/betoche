// Camouflage JSON service surface mounted at the configured route
// prefix (default "/api/feed"). Anything that doesn't match the
// streaming-client signature lands here and gets a normal-looking
// JSON response — discoverable endpoints, OpenAPI shape, proper
// status codes.
//
...
//
// `origin` requires a session-shaped first segment (16-128 char
// urlsafe charset), a method the streaming client uses (GET / POST /
// PUT), and for GET, a non-text/html Accept. Anything else is
// camouflage.
...
// `headers` is a Web `Headers` instance (req.headers in the Edge
// runtime).
