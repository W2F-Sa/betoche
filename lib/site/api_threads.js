...
// Bots / preview crawlers / link-unfurlers that we never want to
// proxy upstream. They produce noisy 400s in the platform's
// access log because the origin (Xray xhttp inbound) refuses
// anything that isn't a real session-bearing client request.
const BOT_UA_RE = /vercel-screenshot|vercel-favicon|HeadlessChrome|Googlebot|bingbot|Slackbot|Twitterbot|facebookexternalhit|Discordbot|WhatsApp|TelegramBot|LinkedInBot|Pinterestbot|Applebot|Bytespider|AhrefsBot|SemrushBot|YandexBot|DuckDuckBot|MJ12bot|crawler|spider/i;
...
export function classifyRequest(path, prefix, headers, method) {
  ...
  // Anything self-identifying as a crawler / bot / preview agent
  // gets the camouflage surface, regardless of method. These are
  // the bulk of the spurious 400s in the function logs (Vercel
  // itself runs vercel-screenshot/1.0 against every new deploy
  // to populate the dashboard preview thumbnail).
  const ua = readHeader(headers, "user-agent");
  if (ua && BOT_UA_RE.test(ua)) {
    return { kind: "camouflage", reason: "bot_ua" };
  }
  ...
  if (m === "GET") {
    ...
    // Browser navigation probes always advertise text/html.
    if (accept.includes("text/html")) {
      return { kind: "camouflage", reason: "html_accept" };
    }
    // The site's own browser `fetch()` calls send the *narrow*
    // accept `application/json` (no `*/*` fallback). xhttp clients
    // always include `*/*` somewhere in the accept list. Treat
    // narrow-JSON-only accepts as camouflage so the cover traffic
    // and the streaming surface can share the same URL shape with
    // no risk of misclassifying a real downlink.
    if (accept.includes("application/json") && !accept.includes("*/*")) {
      return { kind: "camouflage", reason: "json_accept" };
    }
    // No accept header at all → almost certainly not an xhttp
    // client (those always set `*/*`). Send to the camouflage
    // surface to avoid 400-spam in the logs.
    if (!accept) {
      return { kind: "camouflage", reason: "no_accept" };
    }
