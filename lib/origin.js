...
// `alpn: ["h2"]` only and a Let's Encrypt cert that is *valid for a
// different SNI* (e.g. `vercel.com`) than the hostname we connect on.
// To make the bridge "just work":
//
//   * `allowH2: true`      — speak HTTP/2 over the TLS connection
//                            so the ALPN negotiation actually
//                            succeeds. Without this undici only
//                            offers `http/1.1` and Xray's xhttp
//                            inbound rejects the connection with
//                            400 because the negotiated protocol
//                            isn't in its allow-list.
//   * `rejectUnauthorized: false` — accept any cert presented by
//                            the origin, regardless of CN/SAN. The
//                            matching client config uses
//                            `allowInsecure=1` so this is the
//                            intended security posture.
//
// Built into Node ≥18 (undici ships with the runtime) — no
// dependency added.
let originDispatcher = null;
async function getOriginDispatcher() {
  if (originDispatcher) return originDispatcher;
  try {
    const undici = await import("undici");
    originDispatcher = new undici.Agent({
      allowH2: true,
...
export async function streamToOrigin(nodeReq, nodeRes, zoneBase) {
  ...
  let upstream;
  try {
    const dispatcher = await getOriginDispatcher();
    const init = {
      method,
      headers: outHeaders,
