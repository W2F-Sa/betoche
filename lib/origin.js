...
// The xhttp inbound on the origin (Xray) is configured with
// `alpn: ["h2"]` only and a Let's Encrypt cert that may be valid for
// a *different SNI* (e.g. `vercel.com`) than the hostname we connect
// on. To make the bridge "just work":
//
//   * `allowH2: true` — speak HTTP/2 over the TLS connection so the
//     ALPN negotiation actually succeeds. Without this undici only
//     offers `http/1.1` and Xray's xhttp inbound rejects every
//     request with 400 because the negotiated protocol isn't in its
//     allow-list.
//
//   * `rejectUnauthorized: false` — accept any cert presented by
//     the origin, regardless of CN/SAN. The matching client config
//     uses `allowInsecure=1` so this is the intended security
//     posture.
//
// `undici` is a real dependency (see package.json). It is also
// bundled inside Node, but the bundled copy is *not* importable as
// `"undici"`, so we install it explicitly.
const originDispatcher = new Agent({
  allowH2: true,
  connect: {
    rejectUnauthorized: false,
    // Disable SNI validation entirely; we don't care if the cert's
    // SAN matches the hostname we're connecting to.
  },
});
...
export async function streamToOrigin(nodeReq, nodeRes, zoneBase) {
  ...
  try {
    ...
    const detail = process.env.DEBUG_ORIGIN === "1"
      ? `Origin error: ${e?.code || ""} ${e?.message || String(e)}`
      : "Origin temporarily unreachable.";
    return writeJsonError(nodeRes, 503, "service_unavailable", detail, t0);
  }
